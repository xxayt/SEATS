...
# ======================================================================
# Qwen3-Omni: position_embeddings = (B, S, head_dim), standard RoPE + Q/K Norm
# ======================================================================

def apply_token_dropping_qwen3(
    hidden_states: torch.Tensor,
    keep_mask: torch.Tensor,
    causal_mask: Optional[torch.Tensor],
    position_ids: torch.Tensor,
    ...
    # Qwen3: (cos, sin) each (B, S, head_dim) -> index dim 1
    cos, sin = position_embeddings
    cos = cos[:, keep_indices, :]
    sin = sin[:, keep_indices, :]
    position_embeddings = (cos, sin)
...
@torch.no_grad()
def compute_last_token_qk_scores_full_qwen3(
    hidden_states: torch.Tensor,
    decoder_layer,
    position_embeddings: Tuple[torch.Tensor, torch.Tensor],
    text_token_mask: torch.Tensor,
    ...
    bsz, seq_len, _ = normed.size()
    hidden_shape = (bsz, seq_len, -1, attn.head_dim)

    # Q/K Norm applied inline (Qwen3 specific)
    ...
    # Standard RoPE (interleaved M-RoPE already baked into cos/sin)
    cos, sin = position_embeddings
...
@torch.no_grad()
def compute_last_token_attention_importance_qwen3(
    hidden_states: torch.Tensor,
    decoder_layer,
    position_embeddings: Tuple[torch.Tensor, torch.Tensor],
    target_mask: torch.Tensor,
...
def fastv_global_drop_tokens_qwen3(
    hidden_states: torch.Tensor,
    causal_mask: Optional[torch.Tensor],
    position_ids: torch.Tensor,
    position_embeddings: Tuple[torch.Tensor, torch.Tensor],
    ...
    if video_token_before > 0 and video_ratio < 1.0:
        video_scores = compute_last_token_attention_importance_qwen3(
            hidden_states=hidden_states,
            decoder_layer=decoder_layer,
            position_embeddings=position_embeddings,
            target_mask=video_token_mask,
    ...
    if method_name == "fastv":
        ...
        if audio_token_before > 0 and audio_ratio < 1.0:
            audio_scores = compute_last_token_attention_importance_qwen3(
                hidden_states=hidden_states,
                decoder_layer=decoder_layer,
                position_embeddings=position_embeddings,
                target_mask=audio_token_mask,
