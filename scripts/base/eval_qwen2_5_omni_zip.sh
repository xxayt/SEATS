#!/bin/bash
# Common entry script: shared evaluation pipeline for all compression methods.
...
tasks_str=""
ratios_str=""
base_model_name="Qwen2.5-Omni-7B"
...
for tasks in "${tasks_list[@]}"; do
    if [[ "${tasks}" == *"worldsense"* || "${tasks}" == *"daily"* ]]; then
        max_frames=128
    elif [[ "${tasks}" == *"omnivideobench"* ]]; then
        max_frames=256
    else
        max_frames=768
    fi
